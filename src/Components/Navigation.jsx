      ...
      <ul className='flex h-[30%] lg:h-full lg:flex-row flex-col lg:items-center xl:items-center items-start justify-between lg:py-3 list-none gap-0.5 md:gap-1 lg:gap-2 text-sm md:text-base 2xl:text-xl py-8 lg:px-1 px-0.5'>
          {links.map((link) => (
            <Link to={link.link} key={link.name}>
              <li className={`transition-all cursor-pointer py-2 px-4 duration-300 ${location.pathname === link.link ? ' text-[#54D4BD] md:hover:bg-neutral-200 lg:hover:bg-transparent lg:hover:rounded-none hover:rounded-md' : ' text-#1C2221 font-normal hover:text-[#54D4BD] '}`}  onClick={() => handleClick(link.name)}>
                  {link.name}
              </li>
            </Link>
          ))}
